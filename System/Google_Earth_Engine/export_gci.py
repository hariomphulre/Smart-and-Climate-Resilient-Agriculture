...
# Map GCI over ImageCollection
gci_series = s2.map(getGCI)

# Convert each image to a Feature with date and mean GCI
def create_gci_feature(img):
    ...
    gci_value = ee.Number(mean_dict.get('GCI'))
    return ee.Feature(None, {
        'date': img.date().format('YYYY-MM-dd'),
        'gci': gci_value
    })

feature_collection = ee.FeatureCollection(gci_series.map(create_gci_feature))

# Export to GCS
print(f"\nStarting export to GCS bucket: '{GCS_BUCKET_NAME}'...")
task_gcs = ee.batch.Export.table.toCloudStorage(
    collection=feature_collection,
    description='Export_GCI_to_GCS',
...
try:
    ...
    ax.plot(df['date'], df['gci'], marker='o', linestyle='-', color='green', label='Mean GCI')
    ax.set_title('Mean GCI Time Series (Jan 2025 - Apr 2025)', fontsize=16)
